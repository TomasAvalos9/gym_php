...
try {
    $stmt = $pdo->prepare("UPDATE miembros SET estado = ? WHERE idmiembro = ? AND idgimnasio = ?");
    $stmt->execute([$nuevo_estado, $miembro_id, $gimnasio_id]);
    
    // Si el miembro está entrando al gimnasio, incrementar el contador de estado del gimnasio
    if ($nuevo_estado == 1) {
        $stmt = $pdo->prepare("UPDATE gimnasio SET estado = estado + 1 WHERE idgimnasio = ?");
        $stmt->execute([$gimnasio_id]);
    } 
    // Si el miembro está saliendo del gimnasio, decrementar el contador de estado del gimnasio
    else {
        $stmt = $pdo->prepare("UPDATE gimnasio SET estado = GREATEST(0, estado - 1) WHERE idgimnasio = ?");
        $stmt->execute([$gimnasio_id]);
    }
    
    // Redirigir de vuelta a la página de detalle del miembro
    header("Location: detalle_miembro.php?id=" . $miembro_id . "&msg=estado_actualizado");
    exit;
} catch (PDOException $e) {
    // En caso de error, redirigir con mensaje de error
    header("Location: detalle_miembro.php?id=" . $miembro_id . "&error=No se pudo actualizar el estado");
    exit;
}
?>
