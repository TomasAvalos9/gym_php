    ...
    if (empty($error)) {
        try {
            if ($is_editing) {
                $stmt = $pdo->prepare("
                    UPDATE miembros 
                    SET miembro = ?, dni = ?, telefono = ?, correo = ?, idbarrio = ?, idlocalidad = ?, idmembresia = ?, foto = ?
                    WHERE idmiembro = ? AND idgimnasio = ?
                ");
                $stmt->execute([$miembro_nombre, $dni, $telefono, $correo, $idbarrio, $idlocalidad, $idmembresia, $foto, $miembro_id, $gimnasio_id]);
                $success = "Miembro actualizado correctamente";
            } else {
                $password = password_hash('password', PASSWORD_DEFAULT);
                $stmt = $pdo->prepare("
                    INSERT INTO miembros (miembro, dni, telefono, correo, idbarrio, idlocalidad, estado, idmembresia, estado_membresia, idgimnasio, password, foto, deleted) 
                    VALUES (?, ?, ?, ?, ?, ?, 0, ?, 1, ?, ?, ?, 0)
                ");
                $stmt->execute([$miembro_nombre, $dni, $telefono, $correo, $idbarrio, $idlocalidad, $idmembresia, $gimnasio_id, $password, $foto]);
            ...
            <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block sidebar collapse">
                <div class="sidebar-sticky pt-3">
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item"><a class="nav-link active" href="miembros.php"><i class="fas fa-users"></i> Miembros</a></li>
                        <li class="nav-item"><a class="nav-link" href="rutinas.php"><i class="fas fa-dumbbell"></i> Rutinas</a></li>
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
                <div class="container-fluid">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <a href="miembros.php" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Volver</a>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-header bg-dark text-white"><h5 class="mb-0">Formulario de Miembro</h5></div>
                        <div class="card-body">
                            <form method="POST" enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . ($is_editing ? "?id=" . $miembro_id : "")); ?>">
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select" id="idbarrio" name="idbarrio" required>
                                            ...
                                            <?php foreach ($barrios as $barrio): ?>
                                                <option value="<?php echo $barrio['idbarrio']; ?>" <?php echo ($idbarrio == $barrio['idbarrio']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($barrio['barrio']); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select" id="idlocalidad" name="idlocalidad" required>
                                            ...
                                            <?php foreach ($localidades as $localidad): ?>
                                                <option value="<?php echo $localidad['idlocalidad']; ?>" <?php echo ($idlocalidad == $localidad['idlocalidad']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($localidad['localidad']); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    <label for="foto" class="form-label">Foto del Miembro</label>
                                    <input type="file" class="form-control" id="foto" name="foto">
                                    <?php if (!empty($foto)): ?>
                                        <div class="mt-2"><img src="../<?php echo htmlspecialchars($foto); ?>" alt="Foto" width="100" class="rounded shadow"></div>
                                    <?php endif; ?>
                                </div>
